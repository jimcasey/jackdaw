...
**App wiring (REVISED 2026-07-14 by the nav pivot — see [[capture-nav-and-external]]):** `.modelContainer(for: Note.self)` on the app scene (moves to shared `AppModelContainer.shared` at Slice 4′ so the intent shares the store). **NAV FLIPPED: NOT a two-tab shell.** `RootView` = `TriageRootView` (app root; read-only `@Query` list this slice — the persistence-verification surface, GROWS into the real inbox at Slice 3′) with `.sheet(isPresented: $showCapture=true)` AUTO-PRESENTING `CaptureView` as sheet content on launch. CaptureView autosave model unchanged but now sheet content (gains grabber/dismiss; sheet owns its keyboard). **Prune "leave Capture" trigger = sheet `onDismiss`** (single deterministic event) + scenePhase background — this REPLACES the old `.onDisappear`/tab-switch trigger and kills that reliability risk. Keyboard-covers-tab-bar bug now MOOT (sheet owns keyboard). Always launch with Capture presented; endgame = stop auto-presenting once external capture validated. **`VaultProofView` PARKED; Talon core `Jackdaw/Talon/` UNTOUCHED.**
