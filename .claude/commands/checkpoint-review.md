...
2. **Route to the right reviewers** — calibrate to the change, don't over-review
   ...
   - **product-lead** — if the PR changes scope or user-facing capability: is this
     in v1, does it hold the **funnel-not-archive** line (no drift toward
     organizing/browsing/history), and does it stay clear of the dropped **triage**
     stage? Watch for tagging creeping into the capture path at the cost of
     capture speed.
   ...
   `/code-review`. Give each reviewer the diff and any linked issue/spec/ADR.
